...
One straightforward strategy of reducing the \gls{oov} rate is to use frequent lemmas as the basic unit of the lexicon and then derive inflected forms using a morphological generator. Another approach is to limit the lexicon to a set of most frequent inflected forms. It can be done simply by creating the word lexicon from words that occured at least $N$ times in the training corpus. In \cite{nouza2010challenges} it is reported that in case of Czech the second strategy results in better coverage, even with smaller lexicons. It can be further refined by adding a small set of frequent multi-word sequences. This technique has been shown to improve recognition and reduce \gls{oov} rates, although it requires the formulation of automated procedures for finding candidate sequences \cite{kolorenc2006multi}.
\begin{table}[!ht]
...
\subsection{Grammar}
One of the distinguishing grammatical features of Slavic languages is the relatively free word order in sentences. English imposes strict constraints on the order of words, because with its limited inflection, it is the only way of encoding grammatical information. Changing the word order from ``John saw Amy'' to ``Amy saw John'' swaps the subject with the object and therefore completely alters the meaning. In Slavic languages, the role of the word in the sentence can be easily infered from its grammatical category (primarily case \cite{pohl2013using}), so the same grammatical structure can be expressed with different permutations of the words. Each word ordering in Table \ref{table:permutations} is a valid sentence, although some would be preferred stylistically in certain contexts. Changing the order also introduces a slight difference in emphasis. The free word order comes at a price of very strong grammatical agreement between parts of a sentence. The subject must agree in gender, person, and number with the verb. An adjective has to match the noun it describes in terms of gender, number, and case. Most verbo-nominal collocations impose constraints on the case of the noun.
\begin{table}[!ht]
  \caption{Unconstrained word order in Polish.}
    \label{table:permutations}
    \centering
    \begin{tabular*}{.8\linewidth}{@{\extracolsep{\fill}}lll}
      English & Polish \\
      \midrule
      a cat is chasing a bird & kot goni ptaka \\
                              & kot ptaka goni \\
                              & ptaka goni kot \\
                              & ptaka kot goni \\
                              & goni kot ptaka \\
