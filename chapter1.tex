...
Although \gls{asr} has been an active field of research for several decades, for a long time most of the effort has been focused on English. Many important techniques were developed as a result of research programmes financed by the American \gls{darpa} and involving companies and institutions like IBM, AT\&T Bell Labs, Institute for Defense Analysis, Princeton University, and CMU. In the nineties numerous ASR systems originally developed for English had been succesfully ported to other languages: French, German, Japanese, and Mandarin Chinese, among others \cite{besacier2014automatic}. Similarly, the recently introduced \glspl{dnn} models were first trained and benchmarked on English corpora, and later used for other languages \cite{hinton2012deep}. The examples above suggest that similar modeling assumptions can hold for various languages. However, highly inflected languages, such as Polish, still pose a formidable challenge for speech technology researchers, due to their complex grammar, rich inflection, and a large set of phonetically similar prefixes and suffixes. This section describes the features of Polish in the context of \gls{asr} and explains where the main problems arise and how can they can be tackled.
...
\begin{table}[h!]
  \caption{Conjugation of the verb \textit{rozpoznawać} (recognize) in past tense and perfective aspect, with respect to number, person (first, second, third), and gender (masculine, feminine, neuter)}
  \label{table:conjugation}
    ...
    \end{tabular*}
\end{table}
Examples presented in Tables \ref{table:declination} and \ref{table:conjugation} already hint at the most important problem in \gls{asr} of inflected language: the size of the lexicon. While an English noun usually has just two forms (singular and plural), its Polish equivalent can have more than twenty distinct forms (see Table \ref{table:animate}). In case of verbs or adjectives, the difference is even more pronounced. The morphology of Polish is far more complex than what we have seen so far. New words can be created not only by changing the endings, but also by adding multiple prefixes and suffixes, as well as modifying the stem itself. Even negative, comparative, or superlative forms of an adjective are distinct lexical items (in English, they would be created with auxilliary words, such as \textit{not}, \textit{more}, \textit{the most}). This is a serious challenge in building any \gls{asr} system, as the numbers of distinct word-forms can easilly exceed one million. In case of English, 135 vocabulary items are needed to account for half of the Brown Corpus \cite{fagan2011introduction}, and an inventory of 50000 words is enough to achieve 99\% coverage \footnote{In \cite{michel2011quantitative} the size of the English lexicon in 2000 has been statistically estimated as $1022000$, but the distribution of words in a natural language is governed by the Zipf's law, stating that the frequency of any word is inversely proportional to its rank in the frequency table \cite{li1992random}.}. In case of inflected language, the wordbuilding mechanisms allow for producing virtually unlimited number of words, and even a vocabulary on the order of one million items is too large to be processed in real time \cite{nouza2010challenges}. There are several ways of dealing with the tradeoff between the lexicon size and recognition quality.

The most straighforward strategy of reducing the \gls{oov} rate is to limit the lexicon to a set of most frequent word forms. It can be done by simply creating the word lexicon from words that occured at least $N$ times in the training corpus. The size of the lexicon can be controlled to some extent by the value of $N$.
\begin{table}[!ht]
  \caption{Comparison of the noun \textit{student} (student) in English and its inflected forms in Polish}
    ...
    \begin{tabular*}{\linewidth}{@{\extracolsep{\fill}}lll}
      English & Polish \\
      \midrule
      student, students & \multicolumn{2}{p{.8\linewidth}}{\textbf{masculine}: student, studenta, studentowi, studentem, studencie, studenci, studentów, studentom, studentami, studentach} \\
                        & \multicolumn{2}{p{.8\linewidth}}{\textbf{feminine}: studentka, studentki, studentce, studentkę, studentką, studentko, studentki, studentek, studentkom, studentkami, studentkach} \\
    \end{tabular*}
\end{table}

\subsection{Grammar}
\subsection{Phonology}
The vowel system consists of six oral monophthongs and two nasal diphthongs. Table \ref{table:oral} presents the oral vowels, their orthographic transcription, and corresponding \gls{ipa} symbols.
...
The nasal vowels, denoted by letters <ą> and <ę>, are pronounced either as a mid-vowel followed by a nasalised labio-velar or palatal glide (/\textipa{E\super{\~ w}}/, /\textipa{E \super{\~ j}}/, /\textipa{O \super{\~ w}}/), or as a combination of an oral vowel and a nasal consonant, as shown in Table \ref{table:nasal}. From the \gls{asr} perspective, the Polish consonant system is far more complex, mostly due to the distinction between the "rustling" laminal retroflex sounds~(sz,~z,~cz,~dz), the corresponding "humming" alveolo-palatals~(s,~z,~c,~dz), and "hissing" alveolars~(s,~z,~c,~dz). 

\begin{table}[h!]
  \begin{center}
    \caption{Polish nasal vowels \cite{gussmann2007phonology}}
    \label{table:nasal}
    \begin{tabular*}{.6\linewidth}{@{\extracolsep{\fill}}lllll}
      Polish & IPA & example \\
      \midrule
...
\begin{table}[h!]
  \begin{center}
    ...
    \begin{tabular*}{.6\linewidth}{@{\extracolsep{\fill}}lllll}
      ...
      \midrule
      b & \textipa{b} & \textit{bar} /\textipa{bar}/ (bar) & \textbar\\
      c & \textipa{\t{ts}} & \textit{co} /\textipa{\t{ts}O}/ (what)\\
      ć/c(i) & \textipa{\t{tC}} & \textit{ćma} /\textipa{\t{tC}ma}/ (moth), \textit{ciało} /\textipa{\t{tC}awO}/ (body)\\
      d & \textipa{d} & \textit{dom} /\textipa{dOm}/ (home)\\
      dz & \textipa{\t{dz}} & \textit{dzwon} /\textipa{\t{dz}vOn}/ (bell)\\
      dź/dz(i) & \textipa{\t{d\textctz}} & \textit{dźwig} /\textipa{\t{d\textctz}vik}/ (lift), \textit{dzień} /\textipa{\t{d\textctz}e\textltailn}/ (day)\\
      dż & \textipa{\t{dZ}} & \textit{dżem} /\textipa{\t{dZ}Em}/ (jam)\\
      f & \textipa{f} & \textit{fan} /\textipa{fan}/ (fan)\\
      g & \textipa{g} & \textit{gol} /\textipa{gOl}/ (goal)\\
      g(i) & \textipa{g\super{i}} & \textit{magia} /\textipa{mag\super{i}a}/ (magic)\\
      h/ch & \textipa{x} & \textit{hak} /\textipa{xak}/ (hook), \textit{cham} /xam/ (boor)\\
           & \textipa{G} & \textit{niechby} /\textipa{nieGb1}/, \textit{Bohdan} /\textipa{bOGdan}/\\
      h(i)/ch(i) & \textipa{x\super{i}} & \textit{hit} /\textipa{x\super{i}it}/ (hit), \textit{chichot} /\textipa{x\super{i}ixOt}/ (giggle)\\
      j & \textipa{j} & \textit{jak} /\textipa{jak}/ (how)\\
      k & \textipa{k} & \textit{kat} /\textipa{kat}/ (executioner)\\
      k(i) & \textipa{k\super{i}} & \textit{kiedy} /\textipa{k\super{i}jEd1}/ (when)\\
      l & \textipa{l} & \textit{lot} /\textipa{lOt}/ (flight)\\
      ł & \textipa{w} & \textit{łoś} /\textipa{wOC}/ (moose)\\
      m & \textipa{m} & \textit{mapa} /\textipa{mapa}/ (map)\\
      m(i) & \textipa{m\super{i}} & \textit{miód} /\textipa{m\super{i}jut}/ (honey)\\
      n & \textipa{n} & \textit{nos} /\textipa{nOs}/ (nose)\\
      ń/n(i) & \textipa{\textltailn} & \textit{koń} /\textipa{kO\textltailn}/ (horse), \textit{nie} /\textipa{\textltailn E}/ (no)\\
      p & \textipa{p} & \textit{park} /\textipa{park}/ (park)\\
      r & \textipa{r} & \textit{rap} /\textipa{rap}/ (rap)\\
      s & \textipa{s} & \textit{suma} /\textipa{suma}/ (sum)\\
      sz & \textipa{S} & \textit{szum} /\textipa{Sum}/ (noise)\\
      ś/s(i) & \textipa{C} & \textit{śruba} /\textipa{Cruba}/ (screw), \textit{siano} /\textipa{Cano}/ (hay)\\
      t & \textipa{t} & \textit{tak} /\textipa{tak}/ (yes)\\
      w & \textipa{v} & \textit{wór} /\textipa{vur}/ (bag)\\
      z & \textipa{z} & \textit{zero} /\textipa{zErO}/ (zero)\\
      ź/z(i) & \textipa{\textctz} & \textit{źrebak} /\textipa{\textctz rEbak}/ (foal), \textit{zima} /\textipa{\textctz ima}/ (winter)\\
      ż/rz & \textipa{Z} & \textit{żal} /\textipa{Zal}/ (sorrow), \textit{rząd} /\textipa{ZOnt}/ (government)\\
    \end{tabular*}
  \end{center}
\end{table}
