...
\begin{figure}[!ht]
  \centering
    \begin{tikzpicture}[block/.style = {minimum width = 4cm, font = \small}]
      \node [block] (preemphasis) {Preemphasis};
      \node [block, below = of preemphasis] (windowing) {Windowing};
      \node [block, below = of windowing] (dft) {DFT};
      \node [block, below = of dft] (melfilter) {Mel Filters};
      \node [block, below = of melfilter] (log) {Logarithm};
      \node [block, below = of log] (dct) {DCT};
      \node [block, below = of dct] (delta) {Delta Coefficients};
      \path[draw,->]
