...
\begin{equation}
	\gamma(w_{i-n+1}^{i-n})=\frac{\delta_{1}N_{1}(w_{i-n+1}^{i-n}~\bullet)+\delta_{2}N_{2}(w_{i-n+1}^{i-n}~\bullet)+\delta_{3+}N_{3+}(w_{i-n+1}^{i-n}~\bullet)}{\sum_{w_{i}}C(w_{i-n+1}^{i})}
\end{equation}
...
Class-based models address the problem mentioned at the end of the previous section. The idea is to cluster words with similar statistical distributions or linguistic properties into groups. Using classes as the modelling unit reduces the number of parameters, because the number of classes is always smaller than the number of words. An $n$-gram model with $C$ classes has $V-C$ word emission parameters $P(w_{i}|c_{i})$ and $C^{n}-1$ independent $n$-gram parameters of the form $P(c_{i}|c_{i-n+1}^{i-1})$. Consequently, the model is smaller and its parameters are more accurately estimated, because most classes are well represented in the corpus. Class-based $n$-gram models are constructed analogously to word-based models, but words are mapped to equivalence classes. The class mapping $\pi$ can be deterministic or probabilistic. An example of a deterministic mapping is the \texttt{ign} token described in Section \ref{subsection:mle}. A word either appears in the vocabulary or not, so the mapping is unambiguous: 
\begin{equation}
	C:w\mapsto C(w).
	\label{equation:deterministic_class}
\end{equation}
The probability of a word $w_{i}$ given its history $w_{i}^{i-1}$ can be calculated as the product of word emission probability and class $n$-gram probability:
\begin{equation}
	P_{class}(w_{i}|w_{i}^{i-1})=P_{0}(w_{i}|c)P(c_{i}|c)
