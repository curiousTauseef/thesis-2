...
\begin{table}[h!]
	...
	\texttt{%
		...
		\end{tabular*}
\end{table}
Inspecting the Table \ref{table:mle}, which shows an example of applying \gls{mle} on a corpus of four sentences from Monty Python's notorious spam sketch, reveals a major problem. Note that some probability estimates are equal to one and, even worse, some are zeros. This means that because of the chain rule, the model will asign a zero probability to any sequence containing an $n-gram$ which does not occurr in the corpus! Analogously, using such a model to generate random sentences would in many cases result in phrases taken verbatim from the corpus. This is a consequence of data sparsity -- the corpus is too small for the model to properly generalise. Although the example is obviously exaggerated, the problem persists in case of full-sized corpora.
