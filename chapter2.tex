...
Class-based models address the problem mentioned at the end of the previous section. Since word equivalence class is essentially a set of words, using it as the modelling unit reduces the number of parameters. Consequently, the model is smaller and its parameters are more accurately estimated, because most classes are well represented in the corpus. Class-based $n$-gram models are constructed analogously to word-based models, but words are mapped to equivalence classes. The class mapping $\pi$ can be deterministic or probabilistic. An example of a deterministic mapping is the \textit{ign} token described in Section \ref{subsection:mle}. A word either appears in the vocabulary or not, so the mapping is unambiguous. An example of a probabilistic mapping are classes based on \gls{pos} tags.
\begin{equation}
	C:w\mapsto C(w).
