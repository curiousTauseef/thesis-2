...
\end{equation}
	Assuming that language is an ergodic process (that is, it is possible to deduce its statistical properties from a sufficiently long random sample), than for a large $n$, entropy can be approximated by
	\begin{equation}
	...
	\begin{equation}
		\hat{H}=-\frac{1}{n}\log_{2}\hat{P}(w_{1}^{n}),
		\label{equation:entropymodel}
	\end{equation}
	where $w_{1}{n}$ is a sufficiently long, previously unseen sequence called the test text. The probability estimates can never be better than the actual estimates, so $H$ is the lower bound for $\hat{H}$, therefore the.
\subsection{WERR}
