...
where typically $0 < \delta \leq 1$. This method is easy to implement, but has been shown in \cite{Gale94what'swrong} to generally perform poorly. That is why Katz back-off and Chen and Goodman's modified Knesser-Ney smoothing are used in the experimental part.
	...
	\begin{equation}
		...
		\end{cases}
	\end{equation}
	where $\alpha$ is chosen so that the total number of counts in the distribution is unchanged:
	\begin{equation}
	...
	\end{equation}
	The updated probability estimate can be computed from corrected count by normalisation, as in Equation \ref{equation:mle2}.
