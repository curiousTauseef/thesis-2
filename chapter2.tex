...
\begin{equation}
	...
	\end{cases}
\end{equation}
where $d_{c}$ is the discount ratio. The back-off weight $\alpha$ is chosen so that the total number of counts in the distribution is unchanged, i.e. $\sum_{w}C_{\textsc{katz}}(w_{i-1}, w)=\sum_{w}C(w_{i-1}, w)$:
\begin{equation}
	\alpha(w_{i-1})=\frac{1-\sum_{w: C(w_{i-1}, w)>0}P_{\textsc{katz}}(w|w_{i-1})}{\sum_{w: C(w_{i-1}, w)=0}P_{\textsc{mle}}(w)}.
\end{equation}
The updated probability estimate can be computed from corrected count by normalisation:
\begin{equation}
	P_{\textsc{katz}}(w_{i}|w_{i-1})=\frac{C_{\textsc{katz}}(w_{i-1}, w_{i})}{\sum_{w}C_{\textsc{katz}}(w_{i-1}, w)}
\end{equation}
...
\begin{equation}
	...
	\label{equation:dc}
\end{equation}
Katz smoothing for higher-order $n$-gram is defined recursively. It is best illustrated using the trigram example from \cite{whittaker2000statistical}. The updated trigram probability of a word $w_{i}$ given the truncated history $\varphi=w_{i-2}, w_{i-1}$ is given by
\begin{equation}
	\hat{P}(w_{i}|w_{i-2}, w_{i-1})=
	\begin{cases}
		\frac{C(w_{i-2}, w_{i-1}, w_{i})}{C(w_{i-2}, w_{i-1})}& \text{if }C(w_{i-2}, w_{i-1}, w_{i})\\
		d_{C(w_{i-2}, w_{i-1}, w_{i})}\cdot \frac{C(w_{i-2}, w_{i-1}, w_{i})}{C(w_{i-2}, w_{i-1})}& \text{if } 1\leq C(w_{i-2}, w_{i-1}, w_{i})\leq k\\
		\alpha(w_{i-2}, w_{i-1})\cdot \hat{P}(w_{i}|w_{i-1})& \text{if } C(w_{i-2}, w_{i-1}, w_{i})=0,
	\end{cases}
\end{equation}
where:
\begin{equation}
...
\begin{multline}
	P_{\textsc{abs}}(w_{i}|w_{i-(n-1)}, \dots, w_{i-1})=\\
	\frac{max\{C(w_{i-(n-1)}, \dots, w_{i})-\delta, 0\}}{\sum_{w_{i}}C(w_{i-(n-1)}, \dots, w_{i})}+(1-\lambda_{w_{i-(n-1)}, \dots, w_{i}})P_{\textsc{abs}}(w_{i}|w_{i-(n-2)}, \dots, w_{i-1}).
\end{multline}
To make the probability sum to 1, the following condition must be met:
\begin{equation}
	1-\lambda_{w_{i-(n-1)}, \dots, w_{i-1}}=\frac{\delta}{\sum_{w}C(w_{i-(n-1)}, \dots, w)}C_{1+}(w_{i-(n-1)}, \dots, \bullet),
\end{equation}
where $C_{1+}(w_{i-(n-1)}, \dots, w_{i-1}, \bullet)$ denotes, following the convention from \cite{chen1999empirical}, the number of unique words that follow the history $w_{i-(n-1)}, \dots, w_{i-1}$. 
The idea behind Kneser-Ney smoothing is that the lower-order model is only necessary when the count is small or zero in the higher-order model.
