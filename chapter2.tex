...
The probability of a word $w_{i}$ given its history $w_{i-n+1}^{i-1}$ can be calculated as the product of word emission probability and class $n$-gram probability:
\begin{equation}
	P_{\textsc{class}}(w_{i}|w_{i}^{i-1})=P_{0}(w_{i}|c_{i})P(c_{i}|c_{i-n+1}^{i-1})
...
\begin{equation*}
	\begin{aligned}
	c_{i}&=\pi(w_{i}) \\
	c_{i-n+1}^{i-1}&=\pi(w_{i-n+1}), \dots, \pi(w_{i-1}).
\end{aligned}
\end{equation*}
