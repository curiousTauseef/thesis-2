...
\begin{table}[h!]
	\caption{An example WER calculation for a reference phrase \textit{let's recognize speech}}
  \label{table:werr}
    \centering
    \begin{tabular*}{.6\linewidth}{lrrrr}
	        hypothesis               & S & D & I & WER\\
		let's reckon nice beach  & 2 & 1 & 0 & 1\\
		lets wreck a nice speech & 2 & 2 & 0 & $\frac{4}{3}$\\
		lets wreck a nice beach  & 3 & 2 & 0 & $\frac{5}{3}$\\
...
\end{table}
	...
	\begin{equation*}
		P(h)=\alpha P_{\textsc{LM}}+(1-\alpha) P_{\textsc{AM}}
	\end{equation*}
