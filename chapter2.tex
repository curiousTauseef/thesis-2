...
Another way to look at the $n$-gram is to use the notion of equivalence class, already introduced in Section \ref{subsection:lm}. In \cite{jelinek1997statistical}, language modelling is defined as the task of finding the best history classification function:
\begin{equation}
	\Phi:h_{i}\mapsto\varphi_{i}=\Phi(h_{i})=\Phi(w_{1}, \dots, w_{i-1})
\end{equation}
In case of $n$-gram models, the history classification function $\Phi$ is the truncation of word history to the last $n-1$ elements:
\begin{equation}
	\Phi(h_{i})=\Phi(w_{1}, \dots, w_{i-1})=w_{i-(n-1)}, \dots, w_{i-1}
\end{equation}
In an $n$-gram model, two word histories are equivalent if their last $n-1$ elements are identical.
