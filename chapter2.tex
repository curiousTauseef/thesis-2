...
\begin{table}[h!]
	...
	\begin{figure}[htbp]
		\centering
		\includesvg{rnn.svg}
		\caption{A simple \gls{rnn}}
	\end{figure}
	The word vector $w(t)$ has the same size as the vocabulary. It uses 1-of-$N$ coding, meaning that the $i$-th word of the vocabulary is coded by setting the $i$-th element to one and all the other elements to zero \cite{schwenk2005training}. The output layer $y(t)$ has the same dimensionality and represents the probability distribution of the next word. The network is trained using the standard backpropagation algorithm, described in \cite{rumelhart1988learning}. The output of the hidden layer is computed as follows:
	\begin{equation}
		s_{j}(t)=f(\sum_{i}x_{i}(t)u_{ij}),
...
\end{equation}
The values of the ouput layer are calculated using the softmax activation function:
\begin{equation}
