...
\end{equation}
Table \ref{table:mle} shows an example of applying \gls{mle} on a corpus of four sentences from Monty Python's famous spam sketch.
\begin{table}[h!]
	\caption{Calculating bigram probabilities using maximum likelihood estimation \cite{jurafsky2000speech}}
	\label{table:mle}
	\texttt{%
		\begin{tabular}{c}
		...
		\begin{tabular*}{.8\linewidth}{@{\extracolsep{\fill}}lll}
			$P(\texttt{bacon}|\texttt{spam})=\frac{2}{8}$ & rozpoznaliście & rozpoznali \\
			rozpoznałyśmy & rozpoznałyście & rozpoznały \\
			rozpoznałyśmy & rozpoznałyście & rozpoznały \\
		\end{tabular*}
\end{table}
