...
where typically $0 < \delta \leq 1$. This method is easy to implement, but has been shown in \cite{Gale94what'swrong} to generally perform poorly. That is why Katz back-off and Chen and Goodman's modified Knesser-Ney smoothing are used in the experimental part.
	...
	\begin{equation}
		...
		\end{cases}
	\end{equation}
	where $d_{c}$ is the discount coefficient and $\alpha$ is the back-off weight. The discount coefficient $d_{c}$ depends on the the \mbox{$n$-gram} count $c$. The value of $d_{c}$ when $c\leq k$ is chosen so that the resulting discount is proportional to the \mbox{Good-Turing} discount: 
	\begin{equation}
		1-d_{c}=\mu(1-\frac{\hat{c}}{c}).
		\label{equation:constraint1}
	\end{equation}
	Furthermore, the total number of counts discounted in the bigram distribution should be equal to the total number of counts assigned to zero-count bigrams according to the \mbox{Good-Turing} estimate (see Equation \ref{equation:gt}):
	\begin{equation}
		\sum_{c}n_{c}(1-d_{c})c=n_{1}.
		\label{equation:constraint2}
	\end{equation}
	The only solution that satisfies the constraints formulated in Equation \ref{equation:constraint1} and \ref{equation:constraint2} is given in equation \ref{equation:d}. Note that counts larger than some arbitrary treshold $k$ are assumed to be reliable and are not discounted: 
	\begin{equation}
            d_{c}=
            \begin{cases}
	...
	\begin{equation}
		...
		\end{cases}
	\end{equation}
	where $\alpha$ is chosen so that the total number of counts in the distribution is unchanged, i.e. $\sum_{w_{i}}C_{\textsc{katz}}(w_{i})=\sum_{w_{i}}C(w_{i})$::
	\begin{equation}
