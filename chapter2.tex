...
where typically $0 < \delta \leq 1$. This method is easy to implement, but has been shown in \cite{Gale94what'swrong} to generally perform poorly. That is why Katz back-off and Chen and Goodman's modified Knesser-Ney smoothing are used in the experimental part.
	...
	\begin{equation}
		...
		\label{equation:add}
	\end{equation}
	From Equation \ref{equation:add} it follows that the total number of counts that will be assigned to bigrams with zero counts is equal to the number of singletons. The updated probability estimate can be calculated by normalising the updated count by the size of the sample. For an \mbox{$n$-gram} $\alpha$ with $c$ counts:
	\begin{equation}
