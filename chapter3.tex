...
\begin{table}[!htbp]
	...
	\end{tabular*}
\end{table}
The n-gram counts were then smoothed. Several smoothing method were tested~--~Good-Turing, Kneser-Ney, and Chen-Goodman's modified Kneser-Ney. The last two methods were tested in two variants~--~back-off and interpolated. Perplexity of the plain text model calculated on the testing set is presented in Table \ref{table:perplexitysmoothing}. The model discounted using the modified Kneser-Ney method has the lowest perplexity. This method was used for all language models except for the POS models, where the count of counts were too small. 
