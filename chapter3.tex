...
\begin{table}[!htbp]
	...
	\end{tabular*}
\end{table}
The n-gram counts were then smoothed. Several smoothing methods were tested~--~Good-Turing, Kneser-Ney, and Chen-Goodman's modified Kneser-Ney. The last two methods were tested in two variants~--~back-off and interpolated. Perplexity of the plain text model calculated on the testing set is presented in Table \ref{table:perplexitysmoothing}. The model discounted using the modified Kneser-Ney method has the lowest perplexity. This method was used for all language models except for the POS models, where the count of counts were too small. 

\begin{table}[h!]
  \begin{center}
	  \caption{Perplexity of the full plain LM with different smoothing methods~--~Good-Turing, Kneser-Ney (back-off and interpolated), and Chen-Goodman's modified Kneser-Ney (back-off and interpolated), respectively.}
	    \label{table:perplexitysmoothing}
	    \begin{tabular*}{.8\linewidth}{@{\extracolsep{\fill}}l*6r}
		    {} & \multicolumn{1}{c}{GT} & \multicolumn{2}{c}{KN} & \multicolumn{2}{c}{CG}\\
		    {} & \multicolumn{1}{c}{backoff} & \multicolumn{1}{c}{backoff} & \multicolumn{1}{c}{interp} & \multicolumn{1}{c}{backoff} & \multicolumn{1}{c}{interp}\\
...
\begin{figure}[!htbp]
	  \centering
	  \includegraphics[height=10cm, width=15cm]{common_unigrams.png}
...
\begin{figure}[!htbp]
	  \centering
	  \includegraphics[height=10cm, width=15cm]{common_bigrams.png}
...
\begin{figure}[!htbp]
	  \centering
	  \includegraphics[height=10cm, width=15cm]{common_trigrams.png}
	      \caption{Most common bigrams of the plain full corpus.}
