...
Figure \ref{figure:ngramdistribution} present the distribution of n-grams in the full plain corpus on a double logarithmic plot. Zipf's law states that the rank-frequency distribution of words in a natural language corpus is an inverse relation. More formally, the probability mass function of the term frequencies is of the form:
\begin{equation}
	p=\alpha k^{-\beta},
	\label{equation:zipf}
\end{equation}
...
The neural network models were trained on the same datasets as the n-gram models. However, as the WERR results for speech and text n-gram models turned out to be similar (see Section \ref{section:werr}) and the computation times for \gls{rnn} models were extremely large, only the full corpora were used, resulting in four \gls{rnn} models. Twenty percent of phrases from each training corpus were randomly assigned to a validation set, which was used for controlling the learning rate and early stopping. All models were trained using 300 neurons in the hidden layer and two-step \gls{bptt}. For plain and lemma corpora, 500 word classes were used. For POS and GNC models, no additional clustering was neccessary.
